...
### 2026-09-08 개정 — 판정 기준과 순서를 고쳤다

운영 8일치 실측에서 두 가지가 드러났다.

**1. 판정 기준이 좁았다.** "다음 회차 번호를 예측하는가" 만 물어서, 예상번호가 아닌 운세·클릭베이트가 대량 통과했다.

```
황금 재물운 쏟아지는 명당 복권 / 지갑 속 20억 곧 소멸됩니다
1240회 추첨결과.. 명당은 없고..인생여전
```

→ 프롬프트를 **애드센스 심사 기준**으로 바꿨다. 차단 사유 일곱 가지(번호 예측 / 확률 비법 / **운세·기운·부적** / **자극적 클릭베이트** / 판매점 구매 권유 / 도박 부추김 / 무관·광고)를 명시하고, **낱말이 아니라 의도**로 판단하게 했다.

**2. 이미 판정한 것을 매번 다시 물었다.** 기존 키 제거가 LLM 뒤에 있어 하루 494건을 판정했는데 실제 신규는 35건이었다 — **93%가 낭비**였다. 순서를 바꿔 신규만 판정한다.

| | 개정 전 | 개정 후 |
|---|---|---|
| 하루 판정 | 494건 | 약 100건(뉴스 포함) |
| 실행 주기 | 유튜브 6회·뉴스 24회 | **하루 4회**(07·12·17·22시) |
| 화이트리스트 | LLM 면제 | **면제 안 함** |
| 연 비용 | — | **약 680원** |

새벽을 비운 것은 그 시간대에 신규 업로드가 거의 없기 때문이고, **22시를 남긴 것은 토요일 추첨(20:35) 직후 영상이 21:00~22:30 에 몰리기 때문**이다.
