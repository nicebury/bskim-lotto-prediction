...
현재 사용량은 **하루 72회** — 매시간(`0 * * * *`) × 검색어 3개(`로또`, `복권`, `연금복권`). 쿼터의 **0.3%** 다.
...
### ⚠ 이 필터의 대가 — 워커가 멈춘 기간은 영구 결손이다 (2026-08-18 실측)

필터는 **워커가 계속 떠 있다는 전제** 위에서만 옳다. 전제가 깨지면 그 기간의 뉴스는 되찾을 수 없다.

워커가 33일 멈춰 있다 2026-08-18 에 복구됐다. 크론이 정상 발화해 **91건을 담았지만 전부 당일·전날치**였고, **2026-07-16~08-16 의 32일은 0건인 채로 남았다.** 검색 API 는 최신순으로만 주고 필터가 나머지를 버리므로, 크론을 몇 번 더 돌려도 결과는 같다.

| | `lotto` (회차) | `news` (뉴스) |
|---|---|---|
| 지난 것을 다시 부를 주소 | **있다** (회차 번호) | 없다 (최신순 검색뿐) |
| 워커 중단의 결과 | 지연 — catch-up 이 메운다 | **소실** |

되살리려면 `start` 파라미터로 과거 페이지를 훑는 **1회성 백필**이 필요하다. 그것은 아래 **재배포 약관 미확인** 항목에 걸리는 정책 판단이므로 워커가 임의로 시작하지 않는다. 자세한 것은 [[worker-jobs]] 의 catch-up 절.

**필터를 느슨하게 푸는 것은 답이 아니다.** `NEWS_MAX_AGE_DAYS` 를 키우면 평상시 '최신 뉴스' 목록이 며칠씩 밀린다 — 그것을 막으려고 넣은 필터다. 대가를 아는 채로 유지한다.
