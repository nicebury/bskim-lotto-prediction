...
빈도·HOT/COLD·패턴을 **미리 계산해 저장하지 않는다.** 워커는 `lotto_draw` 에 회차를 넣는 것으로 끝나고, 통계는 백엔드가 요청마다 계산한다([[api-contract]] 의 `stats/*`).

실측(2026-07-09): 전 회차 1,231회 번호별 빈도 집계가 **1.54ms**, 테이블 전체가 **248kB** 다. 매주 한 행 늘어난다.

미리 계산하면 세 가지가 나빠진다.

1. **조합이 폭발한다.** `window`(20·50·100·all) × `include_bonus` × 통계 3종. 회차 하나가 추가될 때마다 전부 다시 계산해야 한다. 1.5ms 를 아끼자고.
2. **세션 경계가 무너진다.** 백엔드는 `app_reader` 라 통계 테이블에 **쓸 수 없다.** 그러니 워커가 써야 하는데, 워커의 경계는 "수집해서 쓴다, 계산하지 않는다" 다([[component-boundaries]]). 통계 로직이 워커와 백엔드 두 곳으로 갈라지면, 화면의 숫자와 API 의 숫자가 달라졌을 때 어느 쪽이 맞는지 아무도 모른다.
3. **갱신 순서 문제가 생긴다.** 워커가 회차를 넣고 통계를 다시 만드는 사이에 백엔드가 조회하면 "새 회차 + 옛 통계" 를 본다. 지금은 그런 창이 없다 — 새 회차는 다음 요청부터 즉시 반영된다.

데이터가 커져 계산이 무거워지면 **백엔드 쪽 캐시**가 첫 선택지다. 그래도 부족하면 워커가 소유하는 요약 테이블을 만들되, 이 계약의 변경과 갱신 순서 설계를 함께 한다. **지금은 필요 없다.**
...
**이 테이블은 당분간 비어 있다.** 초안 8.1 이 회차 상세 페이지에 등위별 데이터를 요구한다. 데이터가 없으면 API 는 빈 배열을 반환하고 프론트는 해당 섹션을 숨긴다.

> **비어 있는 이유가 바뀌었다 (2026-07-09).** 종전에는 "수집 소스에 2~5등 정보가 없다" 고 적었으나 **사실이 아니다** — 네이버 위젯은 등위별 1~5등 당첨금·당첨게임 수를 모두 제공한다. 실측으로 확인했다.
>
> 이제 비어 있는 이유는 **수집 정책이 미결이기 때문**이다. 네이버 `robots.txt` 전면 금지와 데이터베이스제작자 권리 검토가 끝나지 않았다. **소스에 데이터가 있다는 것과 담아도 된다는 것은 다르다.** 결정 전까지 이 테이블을 채우지 않는다 — [[dhlottery-blocked]] 의 "수집 정책이 미결이다" 절.
>
> 채우기로 결정하면 용어를 먼저 정리해야 한다. 동행복권과 위젯은 **"당첨게임 수"** 를 쓰는데 `winner_cnt`(당첨자수)와 엄밀히 같지 않다 — 한 사람이 여러 게임을 살 수 있다. 컬럼 코멘트에 반영하거나 컬럼명을 바꾼다(계약 변경).
