...
**매시간 도는 이유는 부하가 아니라 당일 필터 때문이다.** 아래를 본다.

#### 당일 필터 — 발행일 = 실행일

**발행일자(KST)가 수집 실행일자(KST)와 다른 기사는 저장하지 않는다** (`NEWS_SAME_DAY_ONLY=true`).

`sort=date` 로 요청해도 네이버는 며칠 전 기사를 함께 준다 — 실측 94건 중 34건이 과거 기사였다. 오래된 기사가 계속 섞이면 '최신 뉴스' 목록이 며칠씩 밀린다.

세 가지를 지킨다.

- 날짜는 반드시 **KST 로 변환한 뒤** 뗀다. UTC 로 비교하면 한국 시각 오전 9시 이전 발행 기사가 전날로 밀려 전부 버려진다.
...
**감수한 대가.** 마지막 실행 시각과 자정 사이에 발행된 기사는 영영 들어오지 않는다. 다음날 검색되더라도 발행일자가 실행일자와 달라 걸러지기 때문이다. **`NEWS_CRON` 의 실행 간격이 곧 놓치는 창의 크기다** — 하루 3회로 되돌리면 18시~자정 6시간치가 통째로 사라진다. 과거 기사까지 담아야 하면 `NEWS_SAME_DAY_ONLY=false` 로 끈다.
