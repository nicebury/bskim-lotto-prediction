...
class Settings(BaseSettings):
    ...
    # 하루 4회(07·12·17·22시). 2026-09-08 에 매시간에서 줄였다.
    # 새벽에는 신규 기사가 거의 올라오지 않아 22시~07시 공백을 둔다.
    # 22시를 남기는 이유는 토요일 추첨(20:35) 직후 기사가 그 시간대에 몰리기 때문이다.
    # 쿼터가 아니라 LLM 판정 비용과 무의미한 실행을 줄이려는 것이다 —
    # 네이버 쿼터는 25,000 중 12회로 여전히 0.05% 다.
    NEWS_CRON: str = "0 7,12,17,22 * * *"
    ...
    # 2026-09-08: 1 → 2. 매시간에서 하루 4회로 줄이면서 22시~07시 아홉 시간
    # 공백이 생겼다. 1(오늘·어제)로 두면 그 사이 자정을 넘긴 기사가 다음 실행에서
    # 이미 '이틀 전' 이 되어 영영 안 들어온다.
    ...
    # 하루 4회(07·12·17·22시). 뉴스와 같은 시각에 맞춘다 — 운영자가 한 번에
    # 확인할 수 있고, 새벽 실행은 신규가 없어 무의미하다.
    # 22시가 토요일 추첨 영상(21:00~22:30 집중)을 잡는다.
    YOUTUBE_CHANNEL_CRON: str = "0 7,12,17,22 * * *"
    # 4질의 × 6회 = 24 calls / 100. 3시간(32)도 예산 안이지만 재시도·수동 여유를 남긴다.
    # 분을 15 로 어긋내는 것은 정시의 채널 잡과 videos.list 가 겹치지 않게 하기 위해서다
    # (락이 잡별이라 두 잡이 동시에 돈다).
    # 분을 10 으로 어긋낸다. 정시의 채널 잡과 videos.list·LLM 호출이 겹치지
    # 않게 하기 위해서다(락이 잡별이라 두 잡이 동시에 돈다).
    # search.list 는 4질의 × 4회 = 16 calls / 100 으로 여유가 커졌다.
    YOUTUBE_SEARCH_CRON: str = "10 7,12,17,22 * * *"
    ...
    # 2026-09-08 확장. LLM 판정은 비결정적이라 같은 제목도 실행마다 답이 갈린다 —
    # 재검증 후에도 "완벽제외수 100%달성", "층별예상수·제외수" 가 남았다.
    # **명백한 것은 규칙이 잡는다.** 확실하고 비용이 0이며, LLM 은 규칙이 못 잡는
    # 것(신조어·맥락)을 맡아야 한다. 이 관계가 뒤집히면 필터가 운에 좌우된다.
    #
    # 네 갈래다.
    #   번호 지목 — 예상번호·추천번호·제외수·궁합수·고정수…  ('~수' 계열이 핵심이다)
    #   확률 주장 — 고확률·당첨확률·적중률·당첨보장
    #   미신·과장 — 천기누설·재물운·황금기운·신의한수
    #   도구 광고 — 조합기·조합공식
    YOUTUBE_FORBIDDEN_KEYWORDS: str = (
        "예상번호,추천번호,예측번호,예상수,추천수,제외수,궁합수,고정수,필출,"
        "고확률,당첨확률,적중률,1등예측,당첨보장,필승,비법,명당번호,"
        "천기누설,재물운,황금기운,신의한수,대박기운,행운의숫자,"
        "조합공식,조합기"
    )
    ...
    # 20 → 60 (2026-09-08). 20건 배치를 20초 안에 못 끝내는 경우가 실측에서
    # 나왔다(908건 재검증 중 2배치 타임아웃). 실패하면 규칙 결과를 채택하므로
    # 조용히 필터가 약해진다 — 넉넉히 잡는 편이 낫다. 잡은 백그라운드라
    # 60초를 기다려도 사용자를 붙들지 않는다.
    LLM_JUDGE_TIMEOUT_SEC: float = 60.0
