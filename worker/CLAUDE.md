...
- `lotto_draw` · `lotto_prize` · `lotto_news` · `collect_job_log` 테이블의 스키마
...
- **크론 문자열을 코드에 박지 않는다.** `LOTTO_CRON` / `NEWS_CRON` 환경변수에서 읽는다. 수집 딜레이·재시도 횟수도 마찬가지다.
...
| `docs/wiki/90-external/naver-search-api.md` | 뉴스 API. **쿼터 미확인** |
