...
| 이름 | 기본 크론 (KST) | 환경변수 |
|------|----------------|---------|
| `lotto` | 매주 토 21:00 | `LOTTO_CRON` |
| `news` | 매일 06:00 / 12:00 / 18:00 | `NEWS_CRON` |

`news` 의 빈도는 잠정값이다. 네이버 API 일일 쿼터 확인 후 확정한다.
...
**`lotto` 잡이 0건 수집하고 성공한다** — 정상이다. 다음 회차가 아직 추첨되지 않았다는 뜻이다.
