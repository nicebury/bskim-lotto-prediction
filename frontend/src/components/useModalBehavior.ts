 ...
 *
 * ⚠ **포털과 함께 쓸 때는 포털 *안쪽* 컴포넌트에서 부른다.** 이 훅의 effect 는
 *   `ref.current` 가 비어 있으면 아무것도 하지 않고 끝나는데, 포털이 첫 렌더에 `null` 을
 *   돌려주면 바로 그 상태가 된다. 의존성이 그대로라 마운트 뒤에도 다시 돌지 않아 **Esc·
 *   포커스 트랩·스크롤 잠금이 조용히 전부 죽는다**(2026-08-31 에 실제로 겪었다).
 *   증상이 없어 axe 도 잡지 못한다 — 열리기는 열리기 때문이다.
 *
